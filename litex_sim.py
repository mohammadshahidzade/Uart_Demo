...
class CSRHANDLER(Module, AutoCSR):
    def __init__(self,clk,platform):
        platform.add_source("csr_handler.v")
        self.state = CSRStorage(32)
        self.value = CSRStorage(32)
        self.output = CSRStatus(32)
        self.specials += [Instance("csr_handler",
            i_clk = clk,
            i_state = self.state.storage,
            i_value = self.value.storage,
            o_out = self.output.status
        )]
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        if not self.integrated_main_ram_size and with_sdram:
            ...
            self.submodules.csrhandler = csrhandler = CSRHANDLER(self.crg.cd_sys.clk,platform)
            self.add_csr("csrhandler")
